...
{/if}
	...
	<ul class="controlPanel">
		...
		{if $status[item].label == 'Pass'}
			{assign var=status_label value='Complete'}
		{else}
			{assign var=status_label value=$status[item].label}
		{/if}
		<li>
			{if $access.status and $status[item].showlink}
                        	<img src="{$baseurl}/images/{$status[item].icon|default:'default'}.gif" alt="" border="0" width="12" height="12" />&nbsp;<a href="?candID={$candID}&sessionID={$sessionID}&setStageUpdate={$status[item].label}">{$status_label}</a>
			{else}
                        	<img src="{$baseurl}/images/{$status[item].icon|default:'default'}.gif" alt="" border="0" width="12" height="12" />&nbsp;{$status_label}
			{/if}
		</li>
		{/section}
